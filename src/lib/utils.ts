...
export function validateTpSl(params: TpSlValidationParams): string | null {
  ...
  if (takeProfitPrice) {
    ...
    if (tpDeviation > APP_CONFIG.MAX_TPSL_DEVIATION) {
      return `Take Profit is too far from market price. Maximum deviation is ${APP_CONFIG.MAX_TPSL_DEVIATION * 100}%.`
    }
  }
  if (stopLossPrice) {
    const slDeviation = Math.abs(stopLossPrice - marketPrice) / marketPrice
    if (slDeviation > APP_CONFIG.MAX_TPSL_DEVIATION) {
      return `Stop Loss is too far from market price. Maximum deviation is ${APP_CONFIG.MAX_TPSL_DEVIATION * 100}%.`
    }
  }
