...
export default function TradingPage() {
  ...
  useEffect(() => {
    const fetchTradingPairs = async (isInitial: boolean = false) => {
      try {
        ...
        const calculateFundingRate = (symbol: string, priceChange: number, lastPrice: number): number => {
          ...
          const volatilityAdjustment = volatility * APP_CONFIG.FUNDING_VOLATILITY_MULTIPLIER
          const fundingRate = APP_CONFIG.BASE_FUNDING_RATE + (priceChange > 0 ? volatilityAdjustment : -volatilityAdjustment)

          return Math.max(APP_CONFIG.MIN_FUNDING_RATE, Math.min(APP_CONFIG.MAX_FUNDING_RATE, fundingRate))
        }
        ...
        const tradingPairs: TradingPair[] = tickers
          ...
          .sort((a, b) => {
            const majorCoins = APP_CONFIG.MAJOR_COINS as readonly string[]
          ...
          .slice(0, APP_CONFIG.MAX_TRADING_PAIRS)
        ...
        logger.error('Error fetching trading pairs', error as Error)
      } finally {
        if (isInitial) {
          setIsLoading(false)
        }
    ...
    }, APP_CONFIG.TICKER_REFRESH_INTERVAL)
    ...
    return (
      <div className="trading-page h-screen flex items-center justify-center">
        <div className="text-center">
          <div className="text-2xl font-bold text-white mb-2">{APP_CONFIG.APP_NAME}</div>
