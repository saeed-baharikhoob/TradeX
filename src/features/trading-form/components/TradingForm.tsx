...
export const TradingForm = ({ onOrderComplete }: TradingFormProps = {}) => {
  ...
  const [currentLeverage, setCurrentLeverage] = useState<number>(APP_CONFIG.DEFAULT_LEVERAGE)
  ...
  const mockAvailableBalance = APP_CONFIG.MOCK_AVAILABLE_BALANCE
  ...
  const validateOrder = (orderSide: 'buy' | 'sell'): string | null => {
    ...
    if (orderType === 'limit') {
      ...
      if (priceDeviation > APP_CONFIG.MAX_PRICE_DEVIATION) {
        return `Price is too far from market price (${formatPrice(marketPrice)}). Maximum deviation is ${APP_CONFIG.MAX_PRICE_DEVIATION * 100}%.`
      }
    }
